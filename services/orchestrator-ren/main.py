...
class TicketCreateRequest(BaseModel):
    ...
    command: str = Field(..., description="Command to execute (e.g., 'drive', 'scan', 'navigate')")
    params: Dict[str, Any] = Field(default_factory=dict, description="Command parameters")
    priority: TicketPriority = Field(default=TicketPriority.NORMAL, description="Execution priority")
    timeout_seconds: Optional[int] = Field(default=300, description="Max execution time in seconds")
...
class ExecuteRequest(BaseModel):
    ...
    force: bool = Field(default=False, description="Force execution even if already executed")
...
async def root():
    ...
    return {
        ...
        "tickets": {
            ...
            "pending": sum(1 for t in tickets_db.values() if t.status == TicketStatus.PENDING),
            "executing": sum(1 for t in tickets_db.values() if t.status == TicketStatus.EXECUTING),
            "completed": sum(1 for t in tickets_db.values() if t.status == TicketStatus.COMPLETED),
            "failed": sum(1 for t in tickets_db.values() if t.status == TicketStatus.FAILED)
        }
    }
...
async def create_ticket(request: TicketCreateRequest):
    ...
    # Emit telemetry
    telemetry.emit_event("ticket_created", {
        "ticket_id": ticket_id,
        "command": request.command,
...
async def execute_ticket(request: ExecuteRequest):
    ...
    if not request.force and ticket.status in [TicketStatus.COMPLETED, TicketStatus.FAILED]:
        raise HTTPException(
    ...
    telemetry.emit_event("execution_started", {
        "ticket_id": ticket_id,
    ...
    try:
        ...
        telemetry.emit_event("execution_completed", {
            "ticket_id": ticket_id,
            "command": ticket.command,
            "execution_time_ms": execution_time_ms
        })
        telemetry.emit_metric("execution.latency_ms", execution_time_ms, {
            "command": ticket.command,
            "status": "success"
        })
        ...
        telemetry.emit_event("execution_failed", {
            "ticket_id": ticket_id,
            "command": ticket.command,
            "error": str(e),
            "execution_time_ms": execution_time_ms
